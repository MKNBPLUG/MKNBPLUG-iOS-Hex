...
@implementation MKNBHTaskAdopter
...
+ (NSDictionary *)parseReadDataWithCharacteristic:(CBCharacteristic *)characteristic {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA00"]]) {
        ...
        return [self dataParserGetDataSuccess:@{@"state":state} operationID:mk_nbh_connectPasswordOperation];
    }
...
+ (NSDictionary *)parsePacketData:(NSData *)readData {
    ...
    if ([flag isEqualToString:@"01"]) {
        //配置
        mk_nbh_taskOperationID operationID = mk_nbh_defaultTaskOperationID;
        ...
        if ([cmd isEqualToString:@"42"]) {
            //CA证书
            operationID = mk_nbh_taskConfigCAFileOperation;
        }else if ([cmd isEqualToString:@"43"]) {
            //设备证书
            operationID = mk_nbh_taskConfigClientCertOperation;
        }else if ([cmd isEqualToString:@"44"]) {
            //设备私钥
            operationID = mk_nbh_taskConfigClientPrivateKeyOperation;
        }
...
+ (NSDictionary *)parseCustomReadData:(NSString *)content cmd:(NSString *)cmd data:(NSData *)data {
    ...
    if ([cmd isEqualToString:@"4e"]) {
        ...
        operationID = mk_nbh_taskReadDeviceNameOperation;
        ...
        operationID = mk_nbh_taskReadDeviceMacAddressOperation;
    }
...
+ (NSDictionary *)parseCustomConfigData:(NSString *)content cmd:(NSString *)cmd {
    ...
    if ([cmd isEqualToString:@"31"]) {
        //MQTT服务器地址
        operationID = mk_nbh_taskConfigServerHostOperation;
    }else if ([cmd isEqualToString:@"32"]) {
        //MQTT服务器端口号
        operationID = mk_nbh_taskConfigServerPortOperation;
    }else if ([cmd isEqualToString:@"33"]) {
        //MQTT用户名
        operationID = mk_nbh_taskConfigServerUserNameOperation;
    }else if ([cmd isEqualToString:@"34"]) {
        //MQTT密码
        operationID = mk_nbh_taskConfigServerPasswordOperation;
    }else if ([cmd isEqualToString:@"35"]) {
        //MQTT ClientID
        operationID = mk_nbh_taskConfigClientIDOperation;
    }else if ([cmd isEqualToString:@"36"]) {
        //Clean Session
        operationID = mk_nbh_taskConfigServerCleanSessionOperation;
    }else if ([cmd isEqualToString:@"37"]) {
        //Keep Alive
        operationID = mk_nbh_taskConfigServerKeepAliveOperation;
    }else if ([cmd isEqualToString:@"38"]) {
        //MTQQ Qos
        operationID = mk_nbh_taskConfigServerQosOperation;
    }else if ([cmd isEqualToString:@"39"]) {
        //Subscribe Topic
        operationID = mk_nbh_taskConfigSubscibeTopicOperation;
    }else if ([cmd isEqualToString:@"3a"]) {
        //Publish Topic
        operationID = mk_nbh_taskConfigPublishTopicOperation;
    }else if ([cmd isEqualToString:@"3b"]) {
        //LWT 开关
        operationID = mk_nbh_taskConfigLWTStatusOperation;
    }else if ([cmd isEqualToString:@"3c"]) {
        //LWT Qos
        operationID = mk_nbh_taskConfigLWTQosOperation;
    }else if ([cmd isEqualToString:@"3d"]) {
        //LWT Retain
        operationID = mk_nbh_taskConfigLWTRetainOperation;
    }else if ([cmd isEqualToString:@"3e"]) {
        //LWT Topic
        operationID = mk_nbh_taskConfigLWTTopicOperation;
    }else if ([cmd isEqualToString:@"3f"]) {
        //LWT message
        operationID = mk_nbh_taskConfigLWTMessageOperation;
    }else if ([cmd isEqualToString:@"40"]) {
        //DeviceID
        operationID = mk_nbh_taskConfigDeviceIDOperation;
    }else if ([cmd isEqualToString:@"41"]) {
        //加密方式
        operationID = mk_nbh_taskConfigConnectModeOperation;
    }else if ([cmd isEqualToString:@"45"]) {
        //NTP服务器
        operationID = mk_nbh_taskConfigNTPServerHostOperation;
    }else if ([cmd isEqualToString:@"46"]) {
        //时区
        operationID = mk_nbh_taskConfigTimeZoneOperation;
    }else if ([cmd isEqualToString:@"47"]) {
        //APN
        operationID = mk_nbh_taskConfigAPNOperation;
    }else if ([cmd isEqualToString:@"48"]) {
        //APN用户名
        operationID = mk_nbh_taskConfigAPNUserNameOperation;
    }else if ([cmd isEqualToString:@"49"]) {
        //APN密码
        operationID = mk_nbh_taskConfigAPNPasswordOperation;
    }else if ([cmd isEqualToString:@"4a"]) {
        //网络制式
        operationID = mk_nbh_taskConfigNetworkPriorityOperation;
    }else if ([cmd isEqualToString:@"4b"]) {
        //MQTT数据格式
        operationID = mk_nbh_taskConfigDataFormatOperation;
    }else if ([cmd isEqualToString:@"4c"]) {
        //进入产测模式
        operationID = mk_nbh_taskConfigEnterProductTestModeOperation;
    }else if ([cmd isEqualToString:@"4d"]) {
        //工作模式
        operationID = mk_nbh_taskConfigWorkModeOperation;
    }else if ([cmd isEqualToString:@"61"]) {
        //退出debug模式
        operationID = mk_nbh_taskConfigExitDebugModeOperation;
    }
...
+ (NSDictionary *)dataParserGetDataSuccess:(NSDictionary *)returnData operationID:(mk_nbh_taskOperationID)operationID{
    if (!returnData) {
        return @{};
    }
