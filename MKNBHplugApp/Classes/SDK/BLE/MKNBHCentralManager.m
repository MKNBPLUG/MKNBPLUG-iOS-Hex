...
NSString *const mk_nbh_peripheralConnectStateChangedNotification = @"mk_nbh_peripheralConnectStateChangedNotification";
NSString *const mk_nbh_centralManagerStateChangedNotification = @"mk_nbh_centralManagerStateChangedNotification";

NSString *const mk_nbh_deviceDisconnectTypeNotification = @"mk_nbh_deviceDisconnectTypeNotification";

static MKNBHCentralManager *manager = nil;
static dispatch_once_t onceToken;

@interface MKNBHCentralManager ()
...
@property (nonatomic, assign)mk_nbh_centralConnectStatus connectStatus;

@end

@implementation MKNBHCentralManager
...
+ (MKNBHCentralManager *)shared {
    dispatch_once(&onceToken, ^{
        if (!manager) {
            manager = [MKNBHCentralManager new];
        }
    });
    return manager;
}
...
- (void)MKBLEBaseCentralManagerDiscoverPeripheral:(CBPeripheral *)peripheral
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if ([self.delegate respondsToSelector:@selector(mk_nbh_receiveDevice:)]) {
                [self.delegate mk_nbh_receiveDevice:dataModel];
            }
        });
    });
}
...
- (void)MKBLEBaseCentralManagerStartScan {
    if ([self.delegate respondsToSelector:@selector(mk_nbh_startScan)]) {
        [self.delegate mk_nbh_startScan];
    }
}

- (void)MKBLEBaseCentralManagerStopScan {
    if ([self.delegate respondsToSelector:@selector(mk_nbh_stopScan)]) {
        [self.delegate mk_nbh_stopScan];
    }
}
...
- (void)MKBLEBaseCentralManagerStateChanged:(MKCentralManagerState)centralManagerState {
    NSLog(@"蓝牙中心改变");
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_nbh_centralManagerStateChangedNotification object:nil];
}
...
- (void)MKBLEBasePeripheralConnectStateChanged:(MKPeripheralConnectState)connectState {
    ...
    if (connectState == MKPeripheralConnectStateUnknow) {
        self.connectStatus = mk_nbh_centralConnectStatusUnknow;
    }else if (connectState == MKPeripheralConnectStateConnecting) {
        self.connectStatus = mk_nbh_centralConnectStatusConnecting;
    }else if (connectState == MKPeripheralConnectStateConnectedFailed) {
        self.connectStatus = mk_nbh_centralConnectStatusConnectedFailed;
    }else if (connectState == MKPeripheralConnectStateDisconnect) {
        self.connectStatus = mk_nbh_centralConnectStatusDisconnect;
    }
    NSLog(@"当前连接状态发生改变了:%@",@(connectState));
    [[NSNotificationCenter defaultCenter] postNotificationName:mk_nbh_peripheralConnectStateChangedNotification object:nil];
}
...
- (void)peripheral:(CBPeripheral *)peripheral didUpdateValueForCharacteristic:(CBCharacteristic *)characteristic error:(NSError *)error {
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA01"]]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:mk_nbh_deviceDisconnectTypeNotification
                                                            object:nil
    ...
    if ([characteristic.UUID isEqual:[CBUUID UUIDWithString:@"AA05"]]) {
        ...
        if ([self.logDelegate respondsToSelector:@selector(mk_nbh_receiveLog:)]) {
            [self.logDelegate mk_nbh_receiveLog:content];
        }
        return;
    }
}
...
- (mk_nbh_centralManagerStatus )centralStatus {
    return ([MKBLEBaseCentralManager shared].centralStatus == MKCentralManagerStateEnable)
    ? mk_nbh_centralManagerStatusEnable
    : mk_nbh_centralManagerStatusUnable;
}
...
- (void)addTaskWithTaskID:(mk_nbh_taskOperationID)operationID
           characteristic:(CBCharacteristic *)characteristic
              commandData:(NSString *)commandData
             successBlock:(void (^)(id returnData))successBlock
             failureBlock:(void (^)(NSError *error))failureBlock {
    MKNBHOperation <MKBLEBaseOperationProtocol>*operation = [self generateOperationWithOperationID:operationID
...
- (void)addReadTaskWithTaskID:(mk_nbh_taskOperationID)operationID
               characteristic:(CBCharacteristic *)characteristic
                 successBlock:(void (^)(id returnData))successBlock
                 failureBlock:(void (^)(NSError *error))failureBlock {
    MKNBHOperation <MKBLEBaseOperationProtocol>*operation = [self generateReadOperationWithOperationID:operationID
                                                                                        characteristic:characteristic
                                                                                          successBlock:successBlock
                                                                                          failureBlock:failureBlock];
    if (!operation) {
...
- (void)addOperation:(MKNBHOperation *)operation {
    if (!operation) {
        return;
    }
    [[MKBLEBaseCentralManager shared] addOperation:operation];
...
- (BOOL)notifyLogData:(BOOL)notify {
    if (self.connectStatus != mk_nbh_centralConnectStatusConnected || self.peripheral == nil || self.peripheral.nbh_log == nil) {
        return NO;
    }
    [self.peripheral setNotifyValue:notify forCharacteristic:self.peripheral.nbh_log];
    return YES;
}
...
- (void)connectPeripheral:(CBPeripheral *)peripheral
    ...
    MKNBHPeripheral *trackerPeripheral = [[MKNBHPeripheral alloc] initWithPeripheral:peripheral];
    [[MKBLEBaseCentralManager shared] connectDevice:trackerPeripheral sucBlock:^(CBPeripheral * _Nonnull peripheral) {
        ...
        //免密登录
        MKBLEBase_main_safe(^{
            self.connectStatus = mk_nbh_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_nbh_peripheralConnectStateChangedNotification object:nil];
            if (self.sucBlock) {
...
- (void)sendPasswordToDevice {
    ...
    __weak typeof(self) weakSelf = self;
    MKNBHOperation *operation = [[MKNBHOperation alloc] initOperationWithID:mk_nbh_connectPasswordOperation commandBlock:^{
        [[MKBLEBaseCentralManager shared] sendDataToPeripheral:commandData characteristic:[MKBLEBaseCentralManager shared].peripheral.nbh_password type:CBCharacteristicWriteWithResponse];
        ...
        //密码正确
        MKBLEBase_main_safe(^{
            sself.connectStatus = mk_nbh_centralConnectStatusConnected;
            [[NSNotificationCenter defaultCenter] postNotificationName:mk_nbh_peripheralConnectStateChangedNotification object:nil];
            if (sself.sucBlock) {
...
- (MKNBHOperation <MKBLEBaseOperationProtocol>*)generateOperationWithOperationID:(mk_nbh_taskOperationID)operationID
                                                                  characteristic:(CBCharacteristic *)characteristic
                                                                     commandData:(NSString *)commandData
                                                                    successBlock:(void (^)(id returnData))successBlock
                                                                    failureBlock:(void (^)(NSError *error))failureBlock{
    ...
    __weak typeof(self) weakSelf = self;
    MKNBHOperation <MKBLEBaseOperationProtocol>*operation = [[MKNBHOperation alloc] initOperationWithID:operationID commandBlock:^{
...
- (MKNBHOperation <MKBLEBaseOperationProtocol>*)generateReadOperationWithOperationID:(mk_nbh_taskOperationID)operationID
                                                                      characteristic:(CBCharacteristic *)characteristic
                                                                        successBlock:(void (^)(id returnData))successBlock
                                                                        failureBlock:(void (^)(NSError *error))failureBlock{
    if (![[MKBLEBaseCentralManager shared] readyToCommunication]) {
