...
/**
 线程ID
 */
@property (nonatomic, assign)mk_nbh_serverOperationID operationID;
...
@implementation MKNBHMQTTOperation
@synthesize executing = _executing;
@synthesize finished = _finished;

- (void)dealloc{
    NSLog(@"MKNBHMQTTOperation销毁");
}
...
#pragma mark - MKNBHMQTTOperationProtocol
...
- (void)didReceiveMessage:(NSDictionary *)data onTopic:(NSString *)topic {
    ...
    [self dataParserReceivedData:[MKNBHMQTTTaskAdopter parseDataWithJson:data topic:topic]];
}
...
- (void)dataParserReceivedData:(NSDictionary *)dataDic{
    ...
    mk_nbh_serverOperationID operationID = [dataDic[@"operationID"] integerValue];
    if (operationID == mk_nbh_defaultServerOperationID || operationID != self.operationID) {
        return;
    }
