...
@implementation MKNBHUserCredentialsViewModel
@end

@interface MKNBHUserCredentialsView ()
...
@implementation MKNBHUserCredentialsView
...
#pragma mark - setter
- (void)setDataModel:(MKNBHUserCredentialsViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHUserCredentialsViewModel.class]) {
        return;
    }
...
- (MKTextField *)userNameTextField {
    if (!_userNameTextField) {
        ...
        _userNameTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_userCredentials_userNameChanged:)]) {
                [self.delegate nbh_mqtt_userCredentials_userNameChanged:text];
            }
        };
...
- (MKTextField *)passwordTextField {
    if (!_passwordTextField) {
        ...
        _passwordTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_userCredentials_passwordChanged:)]) {
                [self.delegate nbh_mqtt_userCredentials_passwordChanged:text];
            }
        };
