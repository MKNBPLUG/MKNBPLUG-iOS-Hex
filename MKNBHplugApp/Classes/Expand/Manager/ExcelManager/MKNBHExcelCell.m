...
-(void)setMergeCellColumAndRowStr:(NSString *)mergeCellColumAndRowStr
{
    ...
    if(mergeCellColumAndRowStr && [mergeCellColumAndRowStr isKindOfClass:[NSString class]] &&mergeCellColumAndRowStr.length > 0)
    {
        ...
        NSString *rowStr = [MKNBHExcelCell getNumberFromStr:mergeCellColumAndRowStr];
        
        self.mergeRow = [rowStr integerValue];
        
        NSString *column = [MKNBHExcelCell getLetterFromStr:mergeCellColumAndRowStr];
...
-(void)refreshData
{
    ...
    NSString *rowStr = [MKNBHExcelCell getNumberFromStr:r];
    
    self.row = [rowStr integerValue];
    
    NSString *column = [MKNBHExcelCell getLetterFromStr:r];
