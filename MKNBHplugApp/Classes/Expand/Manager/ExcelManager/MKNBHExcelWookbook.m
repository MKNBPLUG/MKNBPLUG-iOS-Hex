...
@implementation MKNBHExcelWookbook
                ...
                if(writeSuccess)
                {
                    ...
                    if(uncompressionSuccess)
                    {
                        ...
                        for(NSInteger i = 0 ; i < array.count ; i ++)
                        {
                            ...
                            MKNBHExcelSheet *sheet = [[MKNBHExcelSheet alloc]init];
                            
                            sheet.sheetId = [[sheetDic objectForKey:@"sheetId"] integerValue];
                            
                            sheet.cellArray = [MKNBHExcelSheet analysisSheetDataWithSheetDic:sheetDic sharedStringsArray:stringValueArray];
...
/**
 ...
 @return 工作表
 */
-(MKNBHExcelSheet *)getSheetWithSheetName:(NSString *)sheetName
{
    MKNBHExcelSheet *sheet = nil;
...
-(NSDictionary *)getSharedStringsWithPath:(NSString *)path
{
    ...
    if(xmlData && xmlData.length > 0)
    {
        ...
        xmlDic = [MKNBHXMLReader dictionaryForXMLData:xmlData error:&error];
    }
    return xmlDic;
}
    ...
    while (fileName = [dirEnum nextObject]) {
        ...
        if ([fileName hasSuffix:@".xml"]) {
            ...
            if(xmlData && xmlData.length > 0)
            {
                ...
                NSInteger sheetId = [[MKNBHExcelCell getNumberFromStr:fileName] integerValue];
                
                NSMutableDictionary *sheetDic = [[NSMutableDictionary alloc]init];
                
                NSDictionary *xmlDic = [MKNBHXMLReader dictionaryForXMLData:xmlData error:&error];
...
-(NSArray *)getSheedNameInfoArrayWithPath:(NSString *)path
{
    ...
    NSDictionary *sheetNameInfoDic = [MKNBHXMLReader dictionaryForXMLData:xmlData error:&error];
