...
@implementation MKNBHExcelSheet
...
/**
 ...
 @return 单元格数据
 */
-(MKNBHExcelCell *)getCellWithColumn:(NSString *)column row:(NSInteger )row error:(NSError **)error
{
    MKNBHExcelCell *cell = nil;

    for(NSInteger i = 0 ; i < self.cellArray.count ;i++)
    {
        MKNBHExcelCell *originCell = [self.cellArray objectAtIndex:i];
 ...
 */
+(NSMutableArray <MKNBHExcelCell *>*)analysisSheetDataWithSheetDic:(NSDictionary *)sheetDic sharedStringsArray:(NSArray *)sharedStringsArray
{
    NSMutableArray <MKNBHExcelCell *>*oneSheetAllCellArray = [self getCellArrayWithSheetDic:sheetDic sharedStringsArray:sharedStringsArray];
...
+(NSMutableArray <MKNBHExcelCell *>*)getCellArrayWithSheetDic:(NSDictionary *)sheetDic sharedStringsArray:(NSArray *)sharedStringsArray
{
    NSMutableArray <MKNBHExcelCell *>*oneSheetAllCellArray = nil;
                ...
                for(NSInteger i = 0 ; i < row.count ;i++)
                {
                    ...
                    if(oneRowDic && [oneRowDic isKindOfClass:[NSDictionary class]])
                    {
                        ...
                        for(NSInteger j = 0 ; j < c.count ; j ++)
                        {
                            ...
                            MKNBHExcelCell *cell = [[MKNBHExcelCell alloc]init];
...
+(NSString *)getMergeCellColumAndRowStrWithCell:(MKNBHExcelCell *)cell mergeCellInfoArray:(NSArray *)mergeCellInfoArray
{
...
+(NSString *)getMergeStrWithMergeInfoDic:(NSDictionary *)mergeInfoDic column:(NSString *)column row:(NSInteger )row
{
    ...
    if(ref && [ref isKindOfClass:[NSString class]] && ref.length > 0)
    {
        ...
        if(array && array.count == 2)
        {
            ...
            NSInteger startRow = [[MKNBHExcelCell getNumberFromStr:startStr] integerValue];
            
            NSInteger endRow = [[MKNBHExcelCell getNumberFromStr:endStr] integerValue];
            
            NSString *tempStartColumnStr = [MKNBHExcelCell getLetterFromStr:startStr];
            
            NSString *tempEndColumnStr = [MKNBHExcelCell getLetterFromStr:endStr];
