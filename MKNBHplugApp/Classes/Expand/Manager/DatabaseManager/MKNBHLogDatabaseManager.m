...
+ (void)insertLogDatas:(NSArray <NSDictionary *>*)logList
    ...
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"MKNBHLogDB")];
    if (![db open]) {
        [self operationInsertFailedBlock:failedBlock];
        return;
    }
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists MKNBHLogDataTable (macAddress text,date text,logDetails text)"];
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKNBHLogDB")] inDatabase:^(FMDatabase *db) {
        
        for (NSDictionary *logDic in logList) {
            BOOL exist = NO;
            FMResultSet * result = [db executeQuery:@"select * from MKNBHLogDataTable where macAddress = ? and date = ?",macAddress,SafeStr(logDic[@"date"])];
            while (result.next) {
                exist = YES;
            }
            if (exist) {
                //存在该设备，更新设备
                [db executeUpdate:@"UPDATE MKNBHLogDataTable SET logDetails = ? where macAddress = ?",SafeStr(logDic[@"logDetails"]),macAddress];
            }else{
                //不存在，插入设备
                [db executeUpdate:@"INSERT INTO MKNBHLogDataTable (macAddress,date,logDetails) VALUES (?,?,?)",macAddress,SafeStr(logDic[@"date"]),SafeStr(logDic[@"logDetails"])];
            }
        }
...
+ (void)readLocalLogsWithMacAddress:(NSString *)macAddress
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKNBHLogDB")] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM MKNBHLogDataTable where macAddress = ?",macAddress];
        while ([result next]) {
            NSDictionary *dic = @{
...
+ (void)deleteDatasWithMacAddress:(NSString *)macAddress
                      ...
                      failedBlock:(void (^)(NSError *error))failedBlock {
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKNBHLogDB")] inDatabase:^(FMDatabase *db) {
        BOOL result = [db executeUpdate:@"DELETE FROM MKNBHLogDataTable where macAddress = ?",macAddress];
        if (!result) {
