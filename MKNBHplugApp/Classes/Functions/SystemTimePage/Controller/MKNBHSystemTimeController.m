...
@interface MKNBHSystemTimeController ()<UITableViewDelegate,
UITableViewDataSource,
MKNBHSystemTimeCellDelegate,
...
@property (nonatomic, strong)MKNBHSystemTimeModel *dataModel;

@end

@implementation MKNBHSystemTimeController

- (void)dealloc {
    NSLog(@"MKNBHSystemTimeController销毁");
    if (self.readTimer) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0)  {
        //Sync Time From NTP
        MKNBHNTPServerController *vc = [[MKNBHNTPServerController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 1) {
        MKNBHSystemTimeCell *cell = [MKNBHSystemTimeCell initCellWithTableView:tableView];
...
#pragma mark - MKNBHSystemTimeCellDelegate
...
- (void)loadSection1Datas {
    MKNBHSystemTimeCellModel *cellModel = [[MKNBHSystemTimeCellModel alloc] init];
    cellModel.index = 0;
...
- (MKNBHSystemTimeModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHSystemTimeModel alloc] init];
    }
    return _dataModel;
}
