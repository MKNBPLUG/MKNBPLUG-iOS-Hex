...
@interface MKNBHEnergyHourlyView ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation MKNBHEnergyHourlyView

- (void)dealloc {
    NSLog(@"MKNBHEnergyHourlyView销毁");
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveHourlyDatas:)
                                                     name:MKNBHReceiveHourlyEnergyDataNotification
                                                   object:nil];
    }
    return self;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKNBHEnergyDataCell *cell = [MKNBHEnergyDataCell initCellWithTableView:tableView];
...
- (void)updateHourlyDatas:(NSDictionary *)dic {
    ...
    for (NSInteger i = 0; i < tempList.count; i ++) {
        ...
        MKNBHEnergyDataCellModel *cellModel = [[MKNBHEnergyDataCellModel alloc] init];
