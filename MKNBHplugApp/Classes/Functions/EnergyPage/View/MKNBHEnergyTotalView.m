...
@interface MKNBHEnergyTotalView ()
...
@implementation MKNBHEnergyTotalView

- (void)dealloc {
    NSLog(@"MKNBHEnergyTotalView销毁");
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(receiveTotalEnergy:)
                                                     name:MKNBHReceiveTotalEnergyDataNotification
                                                   object:nil];
    }
    return self;
}
