...
@interface MKNBHEnergyController ()<UIScrollViewDelegate>

@property (nonatomic, strong)UISegmentedControl *segment;

@property (nonatomic, strong)MKNBHEnergyHourlyView *hourView;

@property (nonatomic, strong)MKNBHEnergyDailyView *dailyView;

@property (nonatomic, strong)MKNBHEnergyTotalView *totalView;

@property (nonatomic, strong)UIScrollView *scrollView;

@property (nonatomic, strong)MKNBHEnergyModel *dataModel;

@property (nonatomic, assign)BOOL isScrolling;

@end

@implementation MKNBHEnergyController

- (void)dealloc {
    NSLog(@"MKNBHEnergyController销毁");
...
- (void)rightButtonMethod {
    ...
    [alertView showAlertWithTitle:@"Reset Energy Data" message:msg notificationName:@"mk_nbh_needDismissAlert"];
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHEnergyController", @"nbh_deleteIcon.png") forState:UIControlStateNormal];
...
- (MKNBHEnergyHourlyView *)hourView {
    if (!_hourView) {
        _hourView = [[MKNBHEnergyHourlyView alloc] init];
    }
    return _hourView;
}

- (MKNBHEnergyDailyView *)dailyView {
    if (!_dailyView) {
        _dailyView = [[MKNBHEnergyDailyView alloc] init];
    }
    return _dailyView;
}

- (MKNBHEnergyTotalView *)totalView {
    if (!_totalView) {
        _totalView = [[MKNBHEnergyTotalView alloc] init];
    }
    return _totalView;
}

- (MKNBHEnergyModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHEnergyModel alloc] init];
    }
    return _dataModel;
}
