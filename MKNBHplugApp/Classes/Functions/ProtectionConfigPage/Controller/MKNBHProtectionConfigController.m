//
//  MKNBHProtectionConfigController.m
...
@interface MKNBHProtectionConfigController ()<UITableViewDelegate,
UITableViewDataSource,
mk_textSwitchCellDelegate,
MKTextFieldCellDelegate>

@property (nonatomic, assign)nbh_protectionConfigType pageType;
...
@property (nonatomic, strong)MKNBHProtectionConfigModel *dataModel;

@end

@implementation MKNBHProtectionConfigController

- (void)dealloc {
    NSLog(@"MKNBHProtectionConfigController销毁");
}

- (instancetype)initWithPageType:(nbh_protectionConfigType)type {
    if (self = [self init]) {
        self.pageType = type;
    }
    return self;
...
- (void)loadSectionDatas {
    if (self.pageType == nbh_protectionConfigType_overload) {
        [self loadOverLoadDatas];
    }else if (self.pageType == nbh_protectionConfigType_overvoltage) {
        [self loadOverVoltageDatas];
    }else if (self.pageType == nbh_protectionConfigType_undervoltage) {
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHProtectionConfigController", @"nbh_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (NSString *)currentTitle {
    switch (self.pageType) {
        case nbh_protectionConfigType_overload:
            return @"Overload Protection";
        case nbh_protectionConfigType_overvoltage:
            return @"Overvoltage Protection";
        case nbh_protectionConfigType_undervoltage:
            return @"Undervoltage Protection";
        case nbh_protectionConfigType_overcurrent:
...
- (MKNBHProtectionConfigModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHProtectionConfigModel alloc] initWithType:self.pageType];
    }
    return _dataModel;
}
...
- (NSString *)noteMsg {
    ...
    if (self.pageType == nbh_protectionConfigType_overvoltage) {
    ...
    if (self.pageType == nbh_protectionConfigType_undervoltage) {
        return @"When the measured voltage exceeds the protection threshold and the duration exceeds the time threshold, the device will turn off automatically.";
    }
    if (self.pageType == nbh_protectionConfigType_overcurrent) {
