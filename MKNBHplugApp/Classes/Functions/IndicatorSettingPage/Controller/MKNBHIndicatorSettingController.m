//
//  MKNBHIndicatorSettingController.m
...
@interface MKNBHIndicatorSettingController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKNBHIndicatorSettingModel *dataModel;

@end

@implementation MKNBHIndicatorSettingController

- (void)dealloc {
    NSLog(@"MKNBHIndicatorSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 3 && indexPath.row == 0) {
        //Indicator setting
        MKNBHIndicatorColorController *vc = [[MKNBHIndicatorColorController alloc] init];
...
- (void)configServerConnecting:(BOOL)isOn {
    ...
    @weakify(self);
    [self.dataModel configServerConnecting:isOn sucBlock:^ {
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)configIndicatorStatus:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    @weakify(self);
    [self.dataModel configIndicatorStatus:isOn sucBlock:^ {
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)configProtectionSignal:(BOOL)isOn {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    @weakify(self);
    [self.dataModel configProtectionSignal:isOn sucBlock:^ {
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)configServerConnected:(NSInteger)status {
    [[MKHudManager share] showHUDWithTitle:@"Config..." inView:self.view isPenetration:NO];
    @weakify(self);
    [self.dataModel configServerConnectedIndicatorStatus:status sucBlock:^ {
        @strongify(self);
        [[MKHudManager share] hide];
...
- (MKNBHIndicatorSettingModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHIndicatorSettingModel alloc] init];
    }
    return _dataModel;
}
