...
@interface MKNBHPowerReportController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKNBHPowerReportModel *dataModel;

@end

@implementation MKNBHPowerReportController

- (void)dealloc {
    NSLog(@"MKNBHPowerReportController销毁");
}
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHPowerReportController", @"nbh_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKNBHPowerReportModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHPowerReportModel alloc] init];
    }
    return _dataModel;
}
