...
@implementation MKNBHModifyServerFooterViewModel
@end
...
@interface MKNBHModifyServerFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKNBHUserCredentialsViewDelegate,
MKNBHModifyServerSSLViewDelegate,
MKNBHModifyServerSettingViewDelegate,
MKNBHModifyServerLWTViewDelegate>
...
@property (nonatomic, strong)MKNBHModifyServerSSLView *sslView;

@property (nonatomic, strong)MKNBHModifyServerLWTView *lwtView;

@property (nonatomic, strong)MKNBHModifyServerSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKNBHModifyServerFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_qosChanged:qosID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:keepAlive textID:0];
    }
}
...
- (void)nbh_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:userName textID:1];
    }
}

- (void)nbh_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKNBHModifyServerSSLViewDelegate
- (void)nbh_mqtt_sslParams_modifyDevice_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:1];
    }
}
...
- (void)nbh_mqtt_sslParams_modifyDevice_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_certificateChanged:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_certificateChanged:certificate];
    }
}
...
- (void)nbh_mqtt_sslParams_modifyDevice_textFieldValueChanged:(NSInteger)index value:(NSString *)value {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:value textID:(index + 5)];
    }
}

#pragma mark - MKNBHModifyServerSettingViewDelegate
/// 输入框内容发生改变
/// @param text 当前输入框内容
...
- (void)nbh_mqtt_modifyDevice_textChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //APN
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:text textID:10];
        }
        return;
    }
    if (textID == 1) {
        //Network_Username
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:text textID:11];
        }
        return;
    }
    if (textID == 2) {
        //Network_Password
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:text textID:12];
        }
        return;
    }
}
...
- (void)nbh_mqtt_modifyDevice_networkPriorityChanged:(NSInteger)priority {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_priorityChanged:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_priorityChanged:priority];
    }
}

#pragma mark - MKNBHModifyServerLWTViewDelegate
- (void)nbh_lwt_modifyDevice_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:2];
    }
}

- (void)nbh_lwt_modifyDevice_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_switchStatusChanged:isOn statusID:3];
    }
}

- (void)nbh_lwt_modifyDevice_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_qosChanged:qosID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_qosChanged:qos qosID:1];
    }
}

- (void)nbh_lwt_modifyDevice_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:text textID:3];
    }
}

- (void)nbh_lwt_modifyDevice_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyMQTT_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_modifyMQTT_textFieldValueChanged:text textID:4];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKNBHModifyServerFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHModifyServerFooterViewModel.class]) {
        return;
    }
    ...
    MKNBHModifyServerSSLViewModel *sslModel = [[MKNBHModifyServerSSLViewModel alloc] init];
    ...
    MKNBHModifyServerLWTViewModel *lwtModel = [[MKNBHModifyServerLWTViewModel alloc] init];
...
- (MKNBHModifyServerSSLView *)sslView {
    if (!_sslView) {
        _sslView = [[MKNBHModifyServerSSLView alloc] init];
...
- (MKNBHModifyServerLWTView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKNBHModifyServerLWTView alloc] init];
...
- (MKNBHModifyServerSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKNBHModifyServerSettingView alloc] init];
