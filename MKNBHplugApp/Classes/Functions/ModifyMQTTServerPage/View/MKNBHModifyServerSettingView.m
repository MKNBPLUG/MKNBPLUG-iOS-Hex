//
//  MKNBHModifyServerSettingView.m
...
@implementation MKNBHModifyServerSettingViewModel
@end

@interface MKNBHModifyServerSettingView ()
...
@implementation MKNBHModifyServerSettingView
...
- (void)priorityButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.priorityList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyDevice_networkPriorityChanged:)]) {
            [self.delegate nbh_mqtt_modifyDevice_networkPriorityChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKNBHModifyServerSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHModifyServerSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)apnField {
    if (!_apnField) {
        ...
        _apnField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyDevice_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_modifyDevice_textChanged:text textID:0];
            }
        };
    }
    return _apnField;
...
- (MKTextField *)netUsernameField {
    if (!_netUsernameField) {
        ...
        _netUsernameField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyDevice_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_modifyDevice_textChanged:text textID:1];
            }
        };
    }
...
- (MKTextField *)netPasswordField {
    if (!_netPasswordField) {
        ...
        _netPasswordField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_modifyDevice_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_modifyDevice_textChanged:text textID:2];
            }
        };
    }
