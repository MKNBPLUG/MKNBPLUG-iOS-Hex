...
@interface MKNBHModifyServerController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKNBHModifyServerFooterViewDelegate>
...
@property (nonatomic, strong)MKNBHModifyServerModel *dataModel;

@property (nonatomic, strong)MKNBHModifyServerFooterView *sslParamsView;

@property (nonatomic, strong)MKNBHModifyServerFooterViewModel *sslParamsModel;

@property (nonatomic, strong)UIView *footerView;

@end

@implementation MKNBHModifyServerController

- (void)dealloc {
    NSLog(@"MKNBHModifyServerController销毁");
...
#pragma mark - MKNBHModifyServerFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)nbh_mqtt_modifyMQTT_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)nbh_mqtt_modifyMQTT_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)nbh_mqtt_modifyMQTT_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //Qos
...
- (void)nbh_mqtt_modifyMQTT_certificateChanged:(NSInteger)certificate {
...
/*
 ...
 10:eMTC
 */
- (void)nbh_mqtt_modifyMQTT_priorityChanged:(NSInteger)priority {
...
- (void)updateLocal {
    [MKNBHDeviceListDatabaseManager updateClientID:self.dataModel.clientID
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_nbh_deviceModifyMQTTServerSuccessNotification"
                                                            object:nil
                                                          userInfo:@{
            @"macAddress":self.dataModel.macAddress,
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHModifyServerController", @"nbh_saveIcon.png") forState:UIControlStateNormal];
...
- (MKNBHModifyServerModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHModifyServerModel alloc] init];
    }
    return _dataModel;
}
...
- (MKNBHModifyServerFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKNBHModifyServerFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKNBHModifyServerFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKNBHModifyServerFooterViewModel alloc] init];
    }
