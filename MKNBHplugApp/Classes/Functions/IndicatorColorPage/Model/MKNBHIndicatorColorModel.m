...
@interface MKNBHIndicatorColorModel ()

@property (nonatomic, assign)mk_nbh_productModel productModel;
...
@implementation MKNBHIndicatorColorModel
...
- (BOOL)readColorDatas {
    ...
    [MKNBHMQTTInterface nbh_readPowerIndicatorColorWithDeviceID:[MKNBHDeviceModeManager shared].deviceID macAddress:[MKNBHDeviceModeManager shared].macAddress topic:[MKNBHDeviceModeManager shared].subscribedTopic sucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configColorDatas {
    __block BOOL success = NO;
    [MKNBHMQTTInterface nbh_configPowerIndicatorColor:self.colorType colorProtocol:self productModel:self.productModel deviceID:[MKNBHDeviceModeManager shared].deviceID macAddress:[MKNBHDeviceModeManager shared].macAddress topic:[MKNBHDeviceModeManager shared].subscribedTopic sucBlock:^ {
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)validParams {
    if (self.colorType == mk_nbh_ledColorTransitionSmoothly || self.colorType == mk_nbh_ledColorTransitionDirectly) {
        NSInteger maxValue = 4416;
        if (self.productModel == mk_nbh_productModel_America) {
            maxValue = 2160;
        }else if (self.productModel == mk_nbh_productModel_UK) {
            maxValue = 3588;
        }
