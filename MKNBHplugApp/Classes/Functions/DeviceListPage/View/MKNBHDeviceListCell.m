...
@implementation MKNBHDeviceListCell

+ (MKNBHDeviceListCell *)initCellWithTableView:(UITableView *)tableView {
    MKNBHDeviceListCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKNBHDeviceListCellIdenty"];
    if (!cell) {
        cell = [[MKNBHDeviceListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKNBHDeviceListCellIdenty"];
    }
    return cell;
}
...
- (void)stateButtonPressed {
    if ([self.delegate respondsToSelector:@selector(nbh_deviceStateChanged:)]) {
        [self.delegate nbh_deviceStateChanged:self.dataModel];
    }
}

- (void)longPressEventAction {
    if ([self.delegate respondsToSelector:@selector(nbh_cellDeleteButtonPressed:)]) {
        [self.delegate nbh_cellDeleteButtonPressed:self.indexPath.row];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKNBHDeviceModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHDeviceModel.class]) {
        return;
    }
    self.deviceNameLabel.text = SafeStr(_dataModel.deviceName);
    
    if (_dataModel.state == MKNBHDeviceModelStateOffline) {
        //是否在线优先级最高
        self.stateLabel.text = @"Offline";
        self.stateLabel.textColor = UIColorFromRGB(0xcccccc);
        [self.stateButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHDeviceListCell", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
        return;
    }
    
    if (_dataModel.overState != MKNBHDeviceOverState_normal) {
        //设备处于过载、过流、过压状态
        self.stateLabel.textColor = [UIColor redColor];
        [self.stateButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHDeviceListCell", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
        if (_dataModel.overState == MKNBHDeviceOverState_overLoad) {
            //过载
            self.stateLabel.text = @"Overload";
        }else if (_dataModel.overState == MKNBHDeviceOverState_overCurrent) {
            //过流
            self.stateLabel.text = @"Overcurrent";
        }else if (_dataModel.overState == MKNBHDeviceOverState_overVoltage) {
            //过压
            self.stateLabel.text = @"Overvoltage";
        }else if (_dataModel.overState == MKNBHDeviceOverState_underVoltage) {
            //欠压
    ...
    //正常状态
    self.stateLabel.textColor = (_dataModel.state == MKNBHDeviceModelStateOn) ? UIColorFromRGB(0x0188cc) : UIColorFromRGB(0xcccccc);
    if (_dataModel.state == MKNBHDeviceModelStateOn) {
        self.stateLabel.text = @"ON";
    }else if (_dataModel.state == MKNBHDeviceModelStateOff) {
        self.stateLabel.text = @"OFF";
    }
    UIImage *stateIcon = (_dataModel.state == MKNBHDeviceModelStateOn) ? LOADICON(@"MKNBHplugApp", @"MKNBHDeviceListCell", @"nbh_switchSelectedIcon.png") : LOADICON(@"MKNBHplugApp", @"MKNBHDeviceListCell", @"nbh_switchUnselectedIcon.png");
...
- (UIImageView *)leftIcon {
    if (!_leftIcon) {
        ...
        _leftIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDeviceListCell", @"nbh_deviceListIcon.png");
    }
    return _leftIcon;
}
...
- (UIImageView *)nextIcon {
    if (!_nextIcon) {
        ...
        _nextIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDeviceListCell", @"mk_nbh_goNextIcon.png");
    }
    return _nextIcon;
}
