...
@interface MKNBHServerForDeviceModel ()
...
@implementation MKNBHServerForDeviceModel

- (instancetype)init {
    if (self = [super init]) {
        _host = [MKNBHMQTTServerManager shared].currentServerParams.host;
        _port = [MKNBHMQTTServerManager shared].currentServerParams.port;
...
- (void)updateValue:(MKNBHServerForDeviceModel *)model {
    if (!model || ![model isKindOfClass:MKNBHServerForDeviceModel.class]) {
        return;
    }
...
- (BOOL)configHost {
    ...
    [MKNBHInterface nbh_configServerHost:self.host sucBlock:^{
        success = YES;
...
- (BOOL)configPort {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configServerPort:[self.port integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configClientID {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configClientID:self.clientID sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configSubscribe {
    ...
    [MKNBHInterface nbh_configSubscibeTopic:topic sucBlock:^{
        success = YES;
...
- (BOOL)configPublish {
    ...
    [MKNBHInterface nbh_configPublishTopic:topic sucBlock:^{
        success = YES;
...
- (BOOL)configCleanSession {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configServerCleanSession:self.cleanSession sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configQos {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configServerQos:self.qos sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configKeepAlive {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configServerKeepAlive:[self.keepAlive integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configUserName {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configServerUserName:self.userName sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configPassword {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configServerPassword:self.password sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configSSLStatus {
    __block BOOL success = NO;
    mk_nbh_connectMode mode = mk_nbh_connectMode_TCP;
    if (self.sslIsOn) {
        if (self.certificate == 0) {
            mode = mk_nbh_connectMode_CACertificate;
        }else if (self.certificate == 1) {
            mode = mk_nbh_connectMode_SelfSignedCertificates;
        }
    }
    [MKNBHInterface nbh_configConnectMode:mode sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configCAFile {
    ...
    [MKNBHInterface nbh_configCAFile:caData sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configClientKey {
    ...
    [MKNBHInterface nbh_configClientPrivateKey:clientKeyData sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configClientCert {
    ...
    [MKNBHInterface nbh_configClientCert:clientCertData sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configDeviceID {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configDeviceID:self.deviceID sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configNTPHost {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configNTPServerHost:self.ntpHost sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configTimeZone {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configTimeZone:(self.timeZone - 24) sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configLWTStatus {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configLWTStatus:self.lwtStatus sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configLWTRetain {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configLWTRetain:self.lwtRetain sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configLWTQos {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configLWTQos:self.lwtQos sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configLWTTopic {
    ...
    [MKNBHInterface nbh_configLWTTopic:topic sucBlock:^{
        success = YES;
        self.lwtTopic = topic;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
...
- (BOOL)configLWTPayload {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configLWTMessage:self.lwtPayload sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configAPN {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configAPN:self.apn sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configNetworkUsername {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configAPNUserName:self.networkUsername sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configNetworkPassword {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configAPNPassword:self.networkPassword sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configNetworkPriority {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configNetworkPriority:self.networkPriority sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configDataformat {
    __block BOOL success = NO;
    [MKNBHInterface nbh_configDataFormat:mk_nbh_dataFormat_hex sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)configWorkMode {
    __block BOOL success = NO;
    mk_nbh_workMode workMode = (self.debugMode ? mk_nbh_workMode_debug : mk_nbh_workMode_rc);
    [MKNBHInterface nbh_configWorkMode:workMode sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readDeviceMac {
    __block BOOL success = NO;
    [MKNBHInterface nbh_readDeviceMacAddressWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)readDeviceName {
    __block BOOL success = NO;
    [MKNBHInterface nbh_readDeviceNameWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
