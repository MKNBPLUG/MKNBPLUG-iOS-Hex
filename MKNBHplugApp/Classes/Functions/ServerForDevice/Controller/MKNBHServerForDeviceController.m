//
//  MKNBHServerForDeviceController.m
...
@interface MKNBHServerForDeviceController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKNBHServerConfigDeviceFooterViewDelegate,
MKCAFileSelectControllerDelegate,
MFMailComposeViewControllerDelegate,
MKNBHImportServerControllerDelegate>
...
@property (nonatomic, strong)MKNBHServerForDeviceModel *dataModel;

@property (nonatomic, strong)MKNBHServerConfigDeviceFooterView *sslParamsView;

@property (nonatomic, strong)MKNBHServerConfigDeviceFooterViewModel *sslParamsModel;
...
@implementation MKNBHServerForDeviceController

- (void)dealloc {
    NSLog(@"MKNBHServerForDeviceController销毁");
...
- (void)leftButtonMethod {
    [self popToViewControllerWithClassName:@"MKNBHScanPageController"];
}
...
#pragma mark - MKNBHServerConfigDeviceFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl    2:lwtStatus  3:lwtRetain  4:debugMode
- (void)nbh_mqtt_serverForDevice_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)nbh_mqtt_serverForDevice_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keepAlive
...
- (void)nbh_mqtt_serverForDevice_qosChanged:(NSInteger)qos qosID:(NSInteger)qosID{
    if (qosID == 0) {
        //qos
...
- (void)nbh_mqtt_serverForDevice_certificateChanged:(NSInteger)certificate {
...
- (void)nbh_mqtt_serverForDevice_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFile
...
- (void)nbh_mqtt_serverForDevice_timeZoneChanged:(NSInteger)timeZone {
...
/*
 ...
 10:eMTC
 */
- (void)nbh_mqtt_serverForDevice_priorityChanged:(NSInteger)priority {
...
- (void)nbh_mqtt_serverForDevice_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
    ...
    if (index == 1) {
        //Import Config File
        MKNBHImportServerController *vc = [[MKNBHImportServerController alloc] init];
        vc.delegate = self;
        [self.navigationController pushViewController:vc animated:YES];
    }
}
...
#pragma mark - MKNBHImportServerControllerDelegate
- (void)nbh_selectedServerParams:(NSString *)fileName {
    [MKNBHExcelDataManager parseDeviceExcel:fileName
                                   sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKNBHServerForDeviceModel *model = [MKNBHServerForDeviceModel mk_modelWithJSON:returnData];
...
- (void)connectStatusChanged {
    if ([MKNBHCentralManager shared].connectStatus == mk_nbh_centralConnectStatusConnected) {
        return;
    }
    ...
    [self.dataModel readDataWithSucBlock:^{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(connectStatusChanged)
                                                     name:mk_nbh_peripheralConnectStateChangedNotification
                                                   object:nil];
    } failedBlock:^(NSError * _Nonnull error) {
        @strongify(self);
        [[MKHudManager share] hide];
...
- (void)configServerParamsComplete {
    [[NSNotificationCenter defaultCenter] removeObserver:self
                                                  ...
                                                  object:nil];
    NSString *topic = @"";
    if (ValidStr([MKNBHMQTTServerManager shared].subscribeTopic)) {
        //查看是否设置了服务器的订阅topic
        topic = [MKNBHMQTTServerManager shared].subscribeTopic;
    }else {
        topic = self.dataModel.publishTopic;
    }
    [self.progressView show];
    [[MKNBHMQTTServerManager shared] subscriptions:@[topic]];
    [[NSNotificationCenter defaultCenter] addObserver:self
...
- (void)connectSuccess {
    ...
    MKNBHDeviceModel *deviceModel = [[MKNBHDeviceModel alloc] init];
    ...
    MKNBHConnectSuccessController *vc = [[MKNBHConnectSuccessController alloc] init];
...
- (void)exportServerConfig {
    ...
    [MKNBHExcelDataManager exportDeviceExcel:self.dataModel
                                    sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHServerForDeviceController", @"nbh_saveIcon.png") forState:UIControlStateNormal];
...
- (MKNBHServerForDeviceModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHServerForDeviceModel alloc] init];
    }
    return _dataModel;
}
...
- (MKNBHServerConfigDeviceFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKNBHServerConfigDeviceFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 380.f)];
...
- (MKNBHServerConfigDeviceFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKNBHServerConfigDeviceFooterViewModel alloc] init];
    }
