//
//  MKNBHServerConfigDeviceFooterView.m
...
@implementation MKNBHServerConfigDeviceFooterViewModel
@end
...
@interface MKNBHServerConfigDeviceFooterView ()<UIScrollViewDelegate,
MKMQTTGeneralParamsViewDelegate,
MKNBHUserCredentialsViewDelegate,
MKNBHMQTTSSLForDeviceViewDelegate,
MKNBHServerConfigDeviceSettingViewDelegate,
MKNBHMQTTLWTForDeviceViewDelegate>
...
@property (nonatomic, strong)MKNBHUserCredentialsView *credentialsView;

@property (nonatomic, strong)MKNBHMQTTSSLForDeviceView *sslView;

@property (nonatomic, strong)MKNBHMQTTLWTForDeviceView *lwtView;

@property (nonatomic, strong)MKNBHServerConfigDeviceSettingView *settingView;

@property (nonatomic, assign)NSInteger index;

@end

@implementation MKNBHServerConfigDeviceFooterView
...
- (void)mk_mqtt_generalParams_cleanSessionStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_switchStatusChanged:isOn statusID:0];
    }
}

- (void)mk_mqtt_generalParams_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_qosChanged:qos qosID:0];
    }
}

- (void)mk_mqtt_generalParams_KeepAliveChanged:(NSString *)keepAlive {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:keepAlive textID:0];
    }
}

#pragma mark - MKNBHUserCredentialsViewDelegate

- (void)nbh_mqtt_userCredentials_userNameChanged:(NSString *)userName {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:userName textID:1];
    }
}

- (void)nbh_mqtt_userCredentials_passwordChanged:(NSString *)password {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:password textID:2];
    }
}

#pragma mark - MKNBHMQTTSSLForDeviceViewDelegate
- (void)nbh_mqtt_sslParams_device_sslStatusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_switchStatusChanged:isOn statusID:1];
    }
}

/// 用户选择了加密方式
/// @param certificate 0:CA certificate     1:Self signed certificates
- (void)nbh_mqtt_sslParams_device_certificateChanged:(NSInteger)certificate {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_certificateChanged:)]) {
        [self.delegate nbh_mqtt_serverForDevice_certificateChanged:certificate];
    }
}

/// 用户点击选择了caFaile按钮
- (void)nbh_mqtt_sslParams_device_caFilePressed {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate nbh_mqtt_serverForDevice_fileButtonPressed:0];
    }
}

/// 用户点击选择了Client Key按钮
- (void)nbh_mqtt_sslParams_device_clientKeyPressed {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate nbh_mqtt_serverForDevice_fileButtonPressed:1];
    }
}

/// 用户点击了Client Cert File按钮
- (void)nbh_mqtt_sslParams_device_clientCertPressed {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_fileButtonPressed:)]) {
        [self.delegate nbh_mqtt_serverForDevice_fileButtonPressed:2];
    }
}

#pragma mark - MKNBHServerConfigDeviceSettingViewDelegate
/// 输入框内容发生改变
/// @param text 当前输入框内容
/// @param textID 0:deviceID 1:NTP URL     2:APN  3:UserName  4:Password
- (void)nbh_mqtt_deviecSetting_textChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //deviceID
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:3];
        }
        return;
    }
    if (textID == 1) {
        //NTP URL
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:4];
        }
        return;
    }
    if (textID == 2) {
        //APN
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:7];
        }
        return;
    }
    if (textID == 3) {
        //UserName
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:8];
        }
        return;
    }
    if (textID == 4) {
        //Password
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
            [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:9];
        }
        return;
    }
}

- (void)nbh_mqtt_deviecSetting_timeZoneChanged:(NSInteger)timeZone {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_timeZoneChanged:)]) {
        [self.delegate nbh_mqtt_serverForDevice_timeZoneChanged:timeZone];
    }
}

- (void)nbh_mqtt_deviecSetting_networkPriorityChanged:(NSInteger)priority {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_priorityChanged:)]) {
        [self.delegate nbh_mqtt_serverForDevice_priorityChanged:priority];
    }
}

- (void)nbh_mqtt_deviecSetting_debugModeChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_switchStatusChanged:isOn statusID:4];
    }
}

/// 底部按钮
/// @param index 0:Export Demo File   1:Import Config File
- (void)nbh_mqtt_deviecSetting_fileButtonPressed:(NSInteger)index {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_bottomButtonPressed:)]) {
        [self.delegate nbh_mqtt_serverForDevice_bottomButtonPressed:index];
    }
}

#pragma mark - MKNBHMQTTLWTForDeviceViewDelegate
- (void)nbh_lwt_statusChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_switchStatusChanged:isOn statusID:2];
    }
}

- (void)nbh_lwt_retainChanged:(BOOL)isOn {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_switchStatusChanged:statusID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_switchStatusChanged:isOn statusID:3];
    }
}

- (void)nbh_lwt_qosChanged:(NSInteger)qos {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_qosChanged:qosID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_qosChanged:qos qosID:1];
    }
}

- (void)nbh_lwt_topicChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:5];
    }
}

- (void)nbh_lwt_payloadChanged:(NSString *)text {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_serverForDevice_textFieldValueChanged:textID:)]) {
        [self.delegate nbh_mqtt_serverForDevice_textFieldValueChanged:text textID:6];
    }
}
...
#pragma mark - setter
- (void)setDataModel:(MKNBHServerConfigDeviceFooterViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHServerConfigDeviceFooterViewModel.class]) {
        return;
    }
    ...
    MKNBHUserCredentialsViewModel *credentialsViewModel = [[MKNBHUserCredentialsViewModel alloc] init];
    ...
    MKNBHMQTTSSLForDeviceViewModel *sslModel = [[MKNBHMQTTSSLForDeviceViewModel alloc] init];
    ...
    MKNBHServerConfigDeviceSettingViewModel *settingModel = [[MKNBHServerConfigDeviceSettingViewModel alloc] init];
    ...
    MKNBHMQTTLWTForDeviceViewModel *lwtModel = [[MKNBHMQTTLWTForDeviceViewModel alloc] init];
...
- (MKNBHUserCredentialsView *)credentialsView {
    if (!_credentialsView) {
        _credentialsView = [[MKNBHUserCredentialsView alloc] init];
...
- (MKNBHMQTTSSLForDeviceView *)sslView {
    if (!_sslView) {
        _sslView = [[MKNBHMQTTSSLForDeviceView alloc] init];
...
- (MKNBHMQTTLWTForDeviceView *)lwtView {
    if (!_lwtView) {
        _lwtView = [[MKNBHMQTTLWTForDeviceView alloc] init];
...
- (MKNBHServerConfigDeviceSettingView *)settingView {
    if (!_settingView) {
        _settingView = [[MKNBHServerConfigDeviceSettingView alloc] init];
