...
@implementation MKNBHMQTTSSLForDeviceViewModel
@end

@interface MKNBHMQTTSSLForDeviceView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKNBHMQTTSSLForDeviceView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_device_caFilePressed)]) {
            [self.delegate nbh_mqtt_sslParams_device_caFilePressed];
        }
        return;
    }
    if (index == 1) {
        //Client Key File
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_device_clientKeyPressed)]) {
            [self.delegate nbh_mqtt_sslParams_device_clientKeyPressed];
        }
        return;
    }
    //Client Cert File
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_device_clientCertPressed)]) {
        [self.delegate nbh_mqtt_sslParams_device_clientCertPressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_device_sslStatusChanged:)]) {
        [self.delegate nbh_mqtt_sslParams_device_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_device_certificateChanged:)]) {
            [self.delegate nbh_mqtt_sslParams_device_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKNBHMQTTSSLForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHMQTTSSLForDeviceViewModel.class]) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKNBHplugApp", @"MKNBHMQTTSSLForDeviceView", @"nbh_switchSelectedIcon.png") : LOADICON(@"MKNBHplugApp", @"MKNBHMQTTSSLForDeviceView", @"nbh_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHMQTTSSLForDeviceView", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
