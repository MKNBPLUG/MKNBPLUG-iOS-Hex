...
@implementation MKNBHMQTTLWTForDeviceViewModel
@end

@interface MKNBHMQTTLWTForDeviceView ()
...
@implementation MKNBHMQTTLWTForDeviceView
...
- (void)lwtButtonPressed {
    ...
    UIImage *lwtIcon = (self.lwtButton.selected ? LOADICON(@"MKNBHplugApp", @"MKNBHMQTTLWTForDeviceView", @"nbh_switchSelectedIcon.png") : LOADICON(@"MKNBHplugApp", @"MKNBHMQTTLWTForDeviceView", @"nbh_switchUnselectedIcon.png"));
    [self.lwtButton setImage:lwtIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(nbh_lwt_statusChanged:)]) {
        [self.delegate nbh_lwt_statusChanged:self.lwtButton.selected];
    }
}

- (void)retainButtonPressed {
    self.retainButton.selected = !self.retainButton.selected;
    UIImage *retainIcon = (self.retainButton.selected ? LOADICON(@"MKNBHplugApp", @"MKNBHMQTTLWTForDeviceView", @"nbh_switchSelectedIcon.png") : LOADICON(@"MKNBHplugApp", @"MKNBHMQTTLWTForDeviceView", @"nbh_switchUnselectedIcon.png"));
    [self.retainButton setImage:retainIcon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(nbh_lwt_retainChanged:)]) {
        [self.delegate nbh_lwt_retainChanged:self.retainButton.selected];
    }
}

- (void)segmentValueChanged {
    if ([self.delegate respondsToSelector:@selector(nbh_lwt_qosChanged:)]) {
        [self.delegate nbh_lwt_qosChanged:self.segment.selectedSegmentIndex];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKNBHMQTTLWTForDeviceViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHMQTTLWTForDeviceViewModel.class]) {
        return;
    }
...
- (UIButton *)lwtButton {
    if (!_lwtButton) {
        ...
        [_lwtButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHMQTTLWTForDeviceView", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (UIButton *)retainButton {
    if (!_retainButton) {
        ...
        [_retainButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHMQTTLWTForDeviceView", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
...
- (MKTextField *)topicTextField {
    if (!_topicTextField) {
        ...
        _topicTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_lwt_topicChanged:)]) {
                [self.delegate nbh_lwt_topicChanged:text];
            }
        };
...
- (MKTextField *)payloadTextField {
    if (!_payloadTextField) {
        ...
        _payloadTextField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_lwt_payloadChanged:)]) {
                [self.delegate nbh_lwt_payloadChanged:text];
            }
        };
