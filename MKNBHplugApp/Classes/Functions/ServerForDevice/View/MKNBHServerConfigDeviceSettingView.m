//
//  MKNBHServerConfigDeviceSettingView.m
...
@implementation MKNBHServerConfigDeviceSettingViewModel
@end

@interface MKNBHServerConfigDeviceSettingView ()
...
@implementation MKNBHServerConfigDeviceSettingView
...
- (void)timeZoneButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.timeZoneList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_timeZoneChanged:)]) {
            [self.delegate nbh_mqtt_deviecSetting_timeZoneChanged:currentRow];
        }
    }];
}
...
- (void)priorityButtonPressed {
    ...
    [pickView showPickViewWithDataList:self.priorityList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_networkPriorityChanged:)]) {
            [self.delegate nbh_mqtt_deviecSetting_networkPriorityChanged:currentRow];
        }
    }];
}
...
- (void)debugButtonPressed {
    ...
    UIImage *icon = (self.debugButton.selected ? LOADICON(@"MKNBHplugApp", @"MKNBHServerConfigDeviceSettingView", @"nbh_switchSelectedIcon.png") : LOADICON(@"MKNBHplugApp", @"MKNBHServerConfigDeviceSettingView", @"nbh_switchUnselectedIcon.png"));
    [self.debugButton setImage:icon forState:UIControlStateNormal];
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_debugModeChanged:)]) {
        [self.delegate nbh_mqtt_deviecSetting_debugModeChanged:self.debugButton.selected];
    }
}

- (void)exportButtonPressed {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate nbh_mqtt_deviecSetting_fileButtonPressed:0];
    }
}

- (void)importButtonPressed {
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_fileButtonPressed:)]) {
        [self.delegate nbh_mqtt_deviecSetting_fileButtonPressed:1];
    }
}

#pragma mark - setter
- (void)setDataModel:(MKNBHServerConfigDeviceSettingViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHServerConfigDeviceSettingViewModel.class]) {
        return;
    }
...
- (MKTextField *)deviceIDField {
    if (!_deviceIDField) {
        ...
        _deviceIDField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_deviecSetting_textChanged:text textID:0];
            }
        };
    }
...
- (MKTextField *)ntpUrlField {
    if (!_ntpUrlField) {
        ...
        _ntpUrlField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_deviecSetting_textChanged:text textID:1];
            }
        };
    }
    return _ntpUrlField;
...
- (MKTextField *)apnField {
    if (!_apnField) {
        ...
        _apnField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_deviecSetting_textChanged:text textID:2];
            }
        };
    }
    return _apnField;
...
- (MKTextField *)netUsernameField {
    if (!_netUsernameField) {
        ...
        _netUsernameField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_deviecSetting_textChanged:text textID:3];
            }
        };
    }
...
- (MKTextField *)netPasswordField {
    if (!_netPasswordField) {
        ...
        _netPasswordField.textChangedBlock = ^(NSString * _Nonnull text) {
            @strongify(self);
            if ([self.delegate respondsToSelector:@selector(nbh_mqtt_deviecSetting_textChanged:textID:)]) {
                [self.delegate nbh_mqtt_deviecSetting_textChanged:text textID:4];
            }
        };
    }
...
- (UIButton *)debugButton {
    if (!_debugButton) {
        ...
        [_debugButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHServerConfigDeviceSettingView", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
