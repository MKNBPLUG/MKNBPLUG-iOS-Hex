...
@interface MKNBHServerForAppController ()<UITableViewDelegate,
UITableViewDataSource,
MKTextFieldCellDelegate,
MKNBHServerConfigAppFooterViewDelegate,
...
@property (nonatomic, strong)MKNBHServerForAppModel *dataModel;

@property (nonatomic, strong)MKNBHServerConfigAppFooterView *sslParamsView;

@property (nonatomic, strong)MKNBHServerConfigAppFooterViewModel *sslParamsModel;
...
@implementation MKNBHServerForAppController

- (void)dealloc {
    NSLog(@"MKNBHServerForAppController销毁");
...
- (void)rightButtonMethod {
    ...
    [[MKNBHMQTTServerManager shared] saveServerParams:self.dataModel];
    ...
    [[MKNBHMQTTServerManager shared] connect];
}
...
#pragma mark - MKNBHServerConfigAppFooterViewDelegate
/// 用户改变了开关状态
/// @param isOn isOn
/// @param statusID 0:cleanSession   1:ssl
- (void)nbh_mqtt_serverForApp_switchStatusChanged:(BOOL)isOn statusID:(NSInteger)statusID {
    if (statusID == 0) {
        //cleanSession
...
- (void)nbh_mqtt_serverForApp_qosChanged:(NSInteger)qos {
...
- (void)nbh_mqtt_serverForApp_textFieldValueChanged:(NSString *)text textID:(NSInteger)textID {
    if (textID == 0) {
        //keep alive
...
- (void)nbh_mqtt_serverForApp_certificateChanged:(NSInteger)certificate {
...
- (void)nbh_mqtt_serverForApp_fileButtonPressed:(NSInteger)fileType {
    if (fileType == 0) {
        //caFaile
...
- (void)nbh_mqtt_serverForApp_bottomButtonPressed:(NSInteger)index {
    if (index == 0) {
        //Export Demo File
...
- (void)nbh_selectedServerParams:(NSString *)fileName {
    [MKNBHExcelDataManager parseAppExcel:fileName
                                sucBlock:^(NSDictionary * _Nonnull returnData) {
        MKNBHServerForAppModel *model = [MKNBHServerForAppModel mk_modelWithJSON:returnData];
...
- (void)serverManagerStateChanged {
    if ([MKNBHMQTTServerManager shared].state == MKMQTTSessionManagerStateConnecting) {
        [[MKHudManager share] showHUDWithTitle:@"Connecting..." inView:self.view isPenetration:NO];
        [self startReceiveTimer];
        self.leftButton.enabled = NO;
        return;
    }
    if ([MKNBHMQTTServerManager shared].state == MKMQTTSessionManagerStateConnected) {
    ...
    if ([MKNBHMQTTServerManager shared].state == MKMQTTSessionManagerStateError) {
        [self connectFailed];
        return;
    }
}
...
//    }
    ...
    [MKNBHExcelDataManager exportAppExcel:self.dataModel
                                 sucBlock:^{
        [[MKHudManager share] hide];
        [self sharedExcel];
    }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHServerForAppController", @"nbh_saveIcon.png") forState:UIControlStateNormal];
...
- (MKNBHServerForAppModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKNBHServerForAppModel alloc] init];
    }
    return _dataModel;
}
...
- (MKNBHServerConfigAppFooterView *)sslParamsView {
    if (!_sslParamsView) {
        _sslParamsView = [[MKNBHServerConfigAppFooterView alloc] initWithFrame:CGRectMake(0, 0, kViewWidth, 230.f)];
...
- (MKNBHServerConfigAppFooterViewModel *)sslParamsModel {
    if (!_sslParamsModel) {
        _sslParamsModel = [[MKNBHServerConfigAppFooterViewModel alloc] init];
    }
