...
@implementation MKNBHMQTTSSLForAppViewModel
@end

@interface MKNBHMQTTSSLForAppView ()<MKMQTTSSLCertificateViewDelegate>
...
@implementation MKNBHMQTTSSLForAppView
...
- (void)mk_fileSelectedButtonPressed:(NSInteger)index {
    if (index == 0) {
        //CA File
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_app_caFilePressed)]) {
            [self.delegate nbh_mqtt_sslParams_app_caFilePressed];
        }
        return;
    }
    //P12证书
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_app_clientFilePressed)]) {
        [self.delegate nbh_mqtt_sslParams_app_clientFilePressed];
    }
}
...
- (void)sslButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_app_sslStatusChanged:)]) {
        [self.delegate nbh_mqtt_sslParams_app_sslStatusChanged:self.sslButton.selected];
    }
}
...
- (void)certificateButtonPressed {
    ...
    [pickView showPickViewWithDataList:dataList selectedRow:index block:^(NSInteger currentRow) {
        ...
        if ([self.delegate respondsToSelector:@selector(nbh_mqtt_sslParams_app_certificateChanged:)]) {
            [self.delegate nbh_mqtt_sslParams_app_certificateChanged:currentRow];
        }
    }];
}

#pragma mark - setter
- (void)setDataModel:(MKNBHMQTTSSLForAppViewModel *)dataModel {
    _dataModel = nil;
    _dataModel = dataModel;
    if (!_dataModel || ![_dataModel isKindOfClass:MKNBHMQTTSSLForAppViewModel.class] || _dataModel.certificate < 0 || _dataModel.certificate > 1) {
        return;
    }
...
- (void)updateSSLButtonIcon {
    UIImage *image = (self.sslButton.selected ? LOADICON(@"MKNBHplugApp", @"MKNBHMQTTSSLForAppView", @"nbh_switchSelectedIcon.png") : LOADICON(@"MKNBHplugApp", @"MKNBHMQTTSSLForAppView", @"nbh_switchUnselectedIcon.png"));
...
- (UIButton *)sslButton {
    if (!_sslButton) {
        ...
        [_sslButton setImage:LOADICON(@"MKNBHplugApp", @"MKNBHMQTTSSLForAppView", @"nbh_switchUnselectedIcon.png") forState:UIControlStateNormal];
