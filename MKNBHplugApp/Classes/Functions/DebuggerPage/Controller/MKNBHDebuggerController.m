...
@interface MKNBHDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_nbh_centralManagerLogDelegate,
MKNBHDebuggerCellDelegate>
...
@property (nonatomic, strong)MKNBHDebuggerButton *deleteButton;

@property (nonatomic, strong)MKNBHDebuggerButton *exportButton;
...
@implementation MKNBHDebuggerController

- (void)dealloc {
    NSLog(@"MKNBHDebuggerController销毁");
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self loadSubViews];
    [MKNBHCentralManager shared].logDelegate = self;
...
- (void)leftButtonMethod {
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKNBHCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKNBHDebuggerCell *cell = [MKNBHDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - mk_nbh_centralManagerLogDelegate
...
#pragma mark - MKNBHDebuggerCellDelegate
- (void)nbh_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKNBHDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_nbh_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_export_disableIcon.png");
    [self.startButton setTitle:@"Stop" forState:UIControlStateNormal];
    [self.contentList removeAllObjects];
    [[MKNBHCentralManager shared] notifyLogData:YES];
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_nbh_syncAnimation"];
}
...
- (void)deleteButtonPressed {
    ...
    [alertView showAlertWithTitle:@"" message:msg notificationName:@"mk_nbh_needDismissAlert"];
}
...
- (void)exitButtonPressed {
    ...
    [MKNBHInterface nbh_exitDebugModeWithSucBlock:^{
...
- (void)readDataFromLocal {
    ...
    [MKNBHLogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)saveConteneDatas {
    ...
    if (self.dataList.count == 0) {
        ...
        for (NSInteger i = 0; i < tempList.count; i ++) {
            MKNBHDebuggerCellModel *tempModel = tempList[i];
            tempModel.index = i;
...
- (void)saveDataListToLocal {
    ...
    [MKNBHLogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction || [MKNBHCentralManager shared].connectStatus != mk_nbh_centralConnectStatusConnected) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKNBHDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_delete_disableIcon.png");
...
- (void)deleteLogDatas {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKNBHLogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKNBHDebuggerCellModel *cellModel = self.dataList[i];
            if (!cellModel.selected) {
...
- (void)showTipsAlert:(NSString *)msg {
    ...
    MKAlertView *alertView = [[MKAlertView alloc] init];
    [alertView addAction:cancelAction];
    [alertView showAlertWithTitle:@"Tips!" message:msg notificationName:@"mk_nbh_needDismissAlert"];
}
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKNBHDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKNBHDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_delete_disableIcon.png");
...
- (MKNBHDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKNBHDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKNBHplugApp", @"MKNBHDebuggerController", @"nbh_export_disableIcon.png");
